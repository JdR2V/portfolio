<script lang="ts">
	...
	const projects = [
		...
		},
		{
			num: '003',
			title: 'Portfolio v1',
			desc: 'Bilingual EN/ES portfolio with GSAP animations and responsive layout.',
			tags: ['React', 'GSAP', 'Vite'],
			url: 'https://github.com/JdR2V/portfolio2',
			live: '#'
		}
	];
</script>
	...
	<div class="mx-auto max-w-5xl px-6">
		<div class="reveal mb-8 flex items-center gap-3">
			<span class="rounded border border-accent/30 px-2 py-0.5 text-xs tracking-[0.2em] text-accent"
				>02</span
			>
			<span class="text-xs tracking-[0.2em] text-text-muted uppercase">selected work</span>
		...
		<h2 class="reveal mb-10 font-display text-4xl font-bold tracking-tight">
			Projects that<br />actually <em class="text-accent">ship</em>
		</h2>
			...
			<div class="reveal mb-4 grid overflow-hidden rounded-xl border border-border md:grid-cols-2">
				...
				>
					<span
                       ...
                       py-0.5 text-xs tracking-wider text-accent">featured</span
					>
				...
				<div class="flex flex-col justify-between gap-4 p-7">
					<div>
						...
						<p class="mb-4 text-xs leading-relaxed text-text-secondary">{project.desc}</p>
